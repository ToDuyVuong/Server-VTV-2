...
public class ProductVariantRequest {
    ...
    public void validate() {
        ...
        if (originalPrice <= 0) {
            throw new BadRequestException("Giá gốc sản phẩm không được nhỏ hơn hoặc bằng 0!");
        }
    ...
    public static void validateAddProductVariantWithImage(List<ProductVariantRequest> productVariantRequests) {
        for (ProductVariantRequest variantRequest : productVariantRequests) {
            if (!variantRequest.isChangeImage()) {
                throw new BadRequestException("Cần chọn hình ảnh cho biến thể sản phẩm khi thêm mới sản phẩm!");
            }
        }
    }
